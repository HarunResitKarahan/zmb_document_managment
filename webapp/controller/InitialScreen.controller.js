    ...
    return BaseController.extend(
      "com.martur.zmbdocumentmanagment.controller.InitialScreen",
      {
        ...
        onDocumentNameValueHelpOkPress: function (oEvent) {
			...
			if (aTokens.length > 0) {
				this.getModel("jsonModel").setProperty("/filterInputValues/documentName", aTokens[0].getText());
			}
			this._closeDialog();
		},
        onDocumentNameFilterBarSearch: function (oEvent) {
			...
			aFilters.push(new Filter({
				filters: [
					...
					new Filter({ path: "fileName", operator: FilterOperator.Contains, value1: sSearchQuery })
				],
				and: false
			}));
